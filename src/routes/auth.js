...
router.post('/auth/register', [
  ...
  body('address').not().isEmpty().withMessage('Invalid address, address can not be left empty'),
  body('password').isLength({min: 6}).withMessage('Password length must be at least 6 characters'),
  body('phoneNumber').isLength({min: 6, max: 9}).withMessage('Phone number should be 9 characters'),
